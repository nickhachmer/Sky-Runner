...
public class UncontrollableOrb : MonoBehaviour
{
    ...
    void Update()
    {
        var playerDistance = Vector2.Distance(transform.position, _playerTransform.position);

        if (playerDistance < _orbRange && playerDistance > 0)
        {
            _isActive = true;
            _playerMovementController.SetActiveOrb(true, transform.position);
            _playerMovementController.CurrentMovementState = PlayerMovementController.MovementState.SlingShotActive;
        }
        else if (_isActive)
        {
            _isActive = false;
